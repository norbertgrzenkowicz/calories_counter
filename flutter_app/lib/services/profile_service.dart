...
class ProfileService {
  ...
  Future<UserProfile?> getUserProfile() async {
    try {
      ...
      developer.log('Fetching profile for user: $userId', name: 'ProfileService');
    ...
    try {
      ...
      try {
        ...
        if (updateResponse != null) {
          developer.log('Profile updated via fallback method', name: 'ProfileService');
        ...
        developer.log('Profile created via fallback method', name: 'ProfileService');
        return UserProfile.fromSupabase(insertResponse);
      } catch (fallbackError) {
        developer.log('Fallback method also failed: $fallbackError', name: 'ProfileService');
  ...
  Future<void> deleteUserProfile() async {
    try {
      ...
      developer.log('Deleting profile for user: $userId', name: 'ProfileService');
      ...
      developer.log('Failed to delete user profile: $e', name: 'ProfileService');
      rethrow;
    }
  }
  ...
  Future<WeightHistory> addWeightEntry(WeightHistory weightEntry) async {
    try {
      ...
      developer.log('Adding weight entry for user: $userId', name: 'ProfileService');
  ...
  Future<List<WeightHistory>> getWeightHistory({
    ...
    try {
      ...
      developer.log('Fetching weight history for user: $userId', name: 'ProfileService');
      ...
      if (startDate != null) {
        queryBuilder = queryBuilder.gte('recorded_date', startDate.toIso8601String().split('T')[0]);
      }

      if (endDate != null) {
        queryBuilder = queryBuilder.lte('recorded_date', endDate.toIso8601String().split('T')[0]);
      }
      ...
      developer.log('Fetched ${response.length} weight entries', name: 'ProfileService');
      return response.map<WeightHistory>((data) => WeightHistory.fromSupabase(data)).toList();
    } catch (e) {
      developer.log('Failed to fetch weight history: $e', name: 'ProfileService');
      rethrow;
    }
  }
  ...
  Future<WeightHistory?> getLatestWeight() async {
    try {
      ...
      developer.log('Failed to fetch latest weight: $e', name: 'ProfileService');
      return null;
    }
  }
  ...
  Future<WeightHistory> updateWeightEntry(WeightHistory weightEntry) async {
    try {
      ...
      developer.log('Updating weight entry: ${weightEntry.id}', name: 'ProfileService');
      ...
      developer.log('Weight entry updated successfully', name: 'ProfileService');
      return WeightHistory.fromSupabase(response);
    } catch (e) {
      developer.log('Failed to update weight entry: $e', name: 'ProfileService');
      rethrow;
    }
  }
  ...
  Future<void> deleteWeightEntry(int weightEntryId) async {
    try {
      ...
      developer.log('Deleting weight entry: $weightEntryId', name: 'ProfileService');
      ...
      developer.log('Weight entry deleted successfully', name: 'ProfileService');
    } catch (e) {
      developer.log('Failed to delete weight entry: $e', name: 'ProfileService');
      rethrow;
    }
  }
  ...
  Future<Map<String, dynamic>> getWeightStatistics({
    ...
    try {
      ...
      return {
        ...
        'dateRange': {
          'start': sortedHistory.first.recordedDate.toIso8601String().split('T')[0],
          'end': sortedHistory.last.recordedDate.toIso8601String().split('T')[0],
        },
      };
    } catch (e) {
      developer.log('Failed to calculate weight statistics: $e', name: 'ProfileService');
      return {
        'totalEntries': 0,
        'weightChange': 0.0,
        'averageWeight': 0.0,
  ...
  Future<bool> isProfileComplete() async {
    try {
      ...
      developer.log('Failed to check profile completeness: $e', name: 'ProfileService');
      return false;
    }
  }
